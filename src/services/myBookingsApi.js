...
const MOCK_UPCOMING = [
  {
    ...
    service: 'Full Groom',
    slot_time: new Date(MOCK_NOW.getTime() + 2 * 24 * 60 * 60 * 1000).toISOString(),
    status: 'confirmed',
    amount: 85,
    can_reschedule: true,
    can_cancel: true,
  },
  {
    ...
    slot_time: new Date(MOCK_NOW.getTime() + 5 * 24 * 60 * 60 * 1000).toISOString(),
    status: 'confirmed',
    amount: 90,
    can_reschedule: true,
    can_cancel: true,
  },
  {
    ...
    slot_time: new Date(MOCK_NOW.getTime() + 18 * 60 * 60 * 1000).toISOString(),
    status: 'confirmed',
    amount: 98,
...
const MOCK_PAST = [
  {
    ...
    service: 'Full Groom',
    slot_time: new Date(MOCK_NOW.getTime() - 7 * 24 * 60 * 60 * 1000).toISOString(),
    status: 'completed',
    amount: 80,
    can_reschedule: false,
    can_cancel: false,
  },
  {
    ...
    slot_time: new Date(MOCK_NOW.getTime() - 21 * 24 * 60 * 60 * 1000).toISOString(),
    status: 'completed',
    amount: 75,
    can_reschedule: false,
    can_cancel: false,
 ...
 *
 * Demo mode (no API_URL): returns mock upcoming and past bookings.
 *
 ...
 *
 * Demo mode: returns the booking with updated slot_time.
 *
...
export async function rescheduleBooking(bookingId, newSlotTime, authToken = null) {
  ...
  if (!response.ok) {
    throw new Error(`rescheduleBooking(${bookingId}) failed: HTTP ${response.status} ${response.statusText}`);
  }
...
export async function cancelBooking(bookingId, authToken = null) {
  ...
  if (!response.ok) {
    throw new Error(`cancelBooking(${bookingId}) failed: HTTP ${response.status} ${response.statusText}`);
  }
